...
def load_metadata(metadata_dir: str): # 核心函数
    ...
    for norm_name, indexes in table_groups.items():
        ...
        if count > 1:
            ...
            column_names[norm_name] = columns
            summary.append(
            ...
            table_names.append(f"Table groups {norm_name} has {count} tables, from the first table '{first_table_name}' to the last '{last_table_name}'. FQN example: {table_fullname}.\n")

        else:
            column_names[first_table_name] = columns
            summary.append(
            f"""Data Definition Language of the table '{first_table_name}':\n{ddl}""")
            # if pd.notna(desc) and desc:
            #     summary.append(f"Description: {desc}\n")
            table_names.append(table_fullname)

        if descriptions and any(d for d in descriptions if d):
            summary.append(f"Descriptions of columns:{descriptions}")

        summary.append("\nSample rows:")
        if sample_rows:
                sample_row = clean(sample_rows[0])
                sample_str = json.dumps(sample_row, indent=2, default=str)
                summary.append(f"{sample_str}\n")
        summary.append("---------------------------")  

    return "\n".join(summary), table_names, json.dumps(column_names, indent=2, ensure_ascii=False)
...
def search_error(error: str):
    # if "Not found" in error:
    #     return f"Please use the correct table names after FROM. {table_names} (Every time when you try to use table names, use the **Fully Qualified Name** in the template)"
    # elif "Unrecognized name" in error:
    #     return f"Please use the correct column names. {column_names}"
    # else:
    vectorstore = FAISS.load_local(
                "/home/hx/RAG/Spider2/spider2-lite/resource/syntax/documents_faiss", 
                default_embeddings, 
                allow_dangerous_deserialization=True
            )
    retriever = vectorstore.as_retriever(search_kwargs={"k": 3})
    relevant_docs = retriever.invoke(error)
