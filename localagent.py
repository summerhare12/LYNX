...
class AutoCodingAgent:
    ...
    def original(self, context: str, data_context: str) -> str:
        if len(data_context) > 400000:
            data_context = data_context[:400000] + "\n...(truncated due to length limit)..."
        prompt_template = '''
        ...
        4. **Formulate the Query**: mind some important rules.
            1. **Date Handling**: 
        ...
        **Output Requirements**:
        1. First, provide a short Analysis of the question with the reasoning strategy above.
        ...
        with get_openai_callback() as cb:
            ...
            # elif exe_flag is False:
                ...
                if "no such table" in error:
                    ...
                    reference = f"Refer to the table names: {str_table_names} "
                elif "no such column" in error or "Unrecognized name" in error or "not exist" in error or re.search(r"Name .*? not found", error):
