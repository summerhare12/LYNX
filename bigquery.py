...
import json
import argparse
from bqagent import AutoCodingAgent
...
def run_task(agent, task, output_dir, client):
    ...
    for task in tqdm(tasks):
        if skipping:
            if task['instance_id'] == last_success_id:
                skipping = False
            continue
        if "sf" in task['instance_id'] or "local" in task["instance_id"]:
            continue
        status, attempts = run_task(agent, task, output_dir, client)
